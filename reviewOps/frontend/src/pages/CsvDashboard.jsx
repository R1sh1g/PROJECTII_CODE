...
export default function CsvDashboard() {
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5 dark:shadow-[0_0_0_1px_rgba(255,255,255,0.06)] dark:backdrop-blur">
          ...
          <label className="block w-full">
            ...
            <input
              ...
              className="
                ...
                dark:border-white/10 dark:bg-black/30 dark:text-white/80
      ...
      {resp?.count != null && (
        <div className="mt-4 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 dark:border-white/10 dark:bg-black/20 dark:text-white/70">
      ...
      {metrics ? (
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-black/20">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold">Top Aspects</h3>
            ...
            <ul className="mt-3 space-y-2">
              {metrics.topAspects.map(([a, c]) => (
                ...
                >
                  <span className="truncate text-sm">{a}</span>
                  <span className="ml-3 inline-flex shrink-0 items-center rounded-lg bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-900 dark:bg-white/10 dark:text-white">
                    {c}
                  </span>
                </li>
              ))}
          ...
          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-black/20">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold">Top Negative Aspects</h3>
              <span className="text-xs text-gray-500 dark:text-white/50">
                Top 10
              </span>
            </div>
            ...
            <ul className="mt-3 space-y-2">
              {metrics.topNegative.map((x) => (
                <li
                  ...
                  className="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-white/5"
                >
                  <span className="truncate text-sm">{x.aspect}</span>

                  <div className="ml-3 flex shrink-0 items-center gap-2">
                    <span className="inline-flex items-center rounded-lg bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-700 dark:bg-rose-500/20 dark:text-rose-100">
                      {x.negative} neg
                    </span>
                    <span className="text-xs text-gray-500 dark:text-white/50">
                      / {x.total} total
        ...
        <div className="mt-5 rounded-2xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-600 dark:border-white/10 dark:bg-black/20 dark:text-white/60">
          Upload a CSV and run analysis to see top aspects and sentiment
          distributions.
        </div>
      )}

      {/* Debug JSON (Rows preview) — this is the "debug json thing" */}
      {resp?.results?.length ? (
        <div className="mt-5">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold">Rows (first 20)</h3>
            <span className="text-xs text-gray-500 dark:text-white/50">
          ...
          <div className="mt-3 overflow-hidden rounded-2xl border border-gray-200 dark:border-white/10">
            <pre className="max-h-[360px] overflow-x-auto bg-gray-900 p-4 text-xs leading-relaxed text-gray-100 dark:bg-black/60 dark:text-white/80">
